...
@app.post("/tickers")
async def get_tickers(tickers: list[str]):
    # Get list of tickers
    returned_tickers = await asyncio.to_thread(fetch_tickers, tickers)
    return {"status": "ok", "returned_tickers": returned_tickers}

@app.post("/tickers/ticker")
async def get_ticker_details(tickers: list[str]):
    # Get list of tickers details
    returned_tickers = await asyncio.to_thread(fetch_returns_sync, tickers)
    return {"status": "ok", "returned_tickers": returned_tickers}

@app.post("/load_db")
async def load_db(tickers: list[str], start_date: date, end_date: date):
    # Run blocking DB work in a thread
    def run():
        for t in tickers:
            load_returns_to_db(t, start_date, end_date)

    await asyncio.to_thread(run)
    return {"status": "ok", "tickers_loaded": tickers}
